...
window.addEventListener("DOMContentLoaded", (event) => {
  ...
  this.updateAuthorImage?.(getTargetAppearance());
  ...
  if (switcher) {
    switcher.addEventListener("click", () => {
      ...
      this.updateAuthorImage?.(targetAppearance);
    });
...
{{ if and (.Site.Params.Image) (.Site.Params.SecondaryImage) }}
{{ $primaryAuthorImage := resources.Get .Site.Params.Image }}
{{ $secondaryAuthorImage := resources.Get .Site.Params.SecondaryImage }}
{{ if and ($primaryAuthorImage) ($secondaryAuthorImage) }}
var updateAuthorImage = (targetAppearance) => {
  var elems;
  elems = document.querySelectorAll("img.mainimage")
  targetLogoPath = 
    targetAppearance == "{{ .Site.Params.DefaultAppearance }}" ?
    "{{ $primaryAuthorImage.RelPermalink }}" : "{{ $secondaryAuthorImage.RelPermalink }}"
  for (const elem of elems) {
    elem.setAttribute("src", targetLogoPath)
  }
}
